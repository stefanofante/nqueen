...
def run_experiments_with_best_ga(
    ...
    Parameters
    - N_values: Sorted board sizes to evaluate.
    - runs_sa: Number of SA runs per N.
    - runs_ga: Number of GA runs per N.
    - bt_time_limit: Backtracking time limit per N (seconds), or None for unlimited.
    - fitness_mode: GA fitness label (e.g., "F1").
    - best_ga_params_for_N: Mapping N -> dict with GA params (pop_size, max_gen, pm, pc, tournament_size).
    - progress_label: Optional label for stdout progress updates.
    - validate: When True, asserts solution correctness and metric consistency.
    - include_bt/include_sa/include_ga: Flags to include/exclude subsystems.
    - bt_solvers: Optional list of BT solver labels to run (default: all discovered).

    Returns
    - ExperimentResults: Nested dict with BT/SA/GA aggregates and raw runs for each N.

    Raises
    - AssertionError: When validate=True and an algorithm reports inconsistent results.
    - ValueError: If requested BT solvers are unknown.
    """
...
def run_experiments_with_best_ga_parallel(
    ...
    Parameters
    - N_values: Board sizes to evaluate.
    - runs_sa/runs_ga: Number of SA/GA runs per N.
    - bt_time_limit: Backtracking time limit (seconds) or None.
    - fitness_mode: GA fitness label (e.g., "F1").
    - best_ga_params_for_N: GA parameters per N (see sequential version).
    - progress_label: Optional label for progress reporting.
    - validate: Enable result assertions for correctness.
    - include_bt/include_sa/include_ga: Subsystem inclusion flags.
    - bt_solvers: Filter list for BT solvers.

    Returns
    - ExperimentResults: Same shape as the sequential variant.

    Raises
    - AssertionError: When validate=True and results are inconsistent.
    - ValueError: If requested BT solvers are unknown.
    """
    results: Any = {"BT": {}, "SA": {}, "GA": {}}
    progress = ProgressPrinter(len(N_values), progress_label) if progress_label else None
...
def run_experiments_parallel(
    ...
    Parameters/Returns
    - Mirrors ``run_experiments_with_best_ga_parallel``. ``runs_bt`` is ignored
      (BT is deterministic in this pipeline) and kept for API compatibility.
    """
